...
int main() 
{
	// Do a bunch of stuff for C++ purposes
	// complicated seed generator for srand -> requires sys/time.h
	timeval tim;
	gettimeofday(&tim, NULL);
	unsigned int seed = ( getpid() ^ ( tim.tv_sec*(tim.tv_usec) ) );
	srand ( seed );


	// first delete the file
	if ( remove( "./results/xdata.dat" ) == -1 ) 
		cout << "Error deleting file\n";
	else
		cout << "File successfully deleted\n";


	// open a file to later output the x_coord and MoveX data for gnuplot to plot them
	// make sure ./results directory exists
	ofstream x_data;
	x_data.open("./results/xdata.dat", ios::out| ios::app);
	if(x_data.fail())
	{
	...
	// Start the simulation important stuff
	// set the time interval for the simulation and # of steps
	aScalar timeStep = 0.01;
	int steps = 6000;  		// important to keep 8000 steps for now for the supervid
	...
	for ( int n = 0; n < steps; n++ ) 
	{
		...
		for  ( int i = 0; i < beadNumber; i++ )
		{
			...
			// output the position of the beads to the data file
			results = unit_l * posit_move;
			x_data << results << endl;
